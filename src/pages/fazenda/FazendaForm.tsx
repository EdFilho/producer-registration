...
import { createPropriedade, updatePropriedade, fetchPropriedadeById } from '../../store/propriedadeRuralSlice';
import { ActionButton, NotificationModal } from '../../components/shared';
import { PropriedadeRuralFormData } from '../../types/propriedadeRural';
import { SafraFormData, Safra } from '../../types/safra';
...
const FazendaForm: React.FC = () => {
  ...
  useEffect(() => {
    if (currentPropriedade && isEditing) {
      setFazendaData({
        ...
        areaTotalHectares: String(currentPropriedade.areaTotalHectares),
        areaAgricultavelHectares: String(currentPropriedade.areaAgricultavelHectares),
        areaVegetacaoHectares: String(currentPropriedade.areaVegetacaoHectares)
      });
    }
  ...
  const validateFazenda = (): boolean => {
    ...
    const areaTotal = parseFloat(fazendaData.areaTotalHectares) || 0;
    if (areaTotal <= 0) {
      showNotification('error', 'Erro de Validação', 'Área total deve ser maior que zero');
      return false;
    }

    const areaAgricultavel = parseFloat(fazendaData.areaAgricultavelHectares) || 0;
    const areaVegetacao = parseFloat(fazendaData.areaVegetacaoHectares) || 0;
  ...
  const handleEditSafra = (safra: Safra) => {
    setIsAddingSafra(true);
    setSafraData({ ano: safra.ano.toString(), nome: safra.nome, culturasPlantadas: safra.culturasPlantadas });
  ...
  const handleSaveSafra = async () => {
    ...
    try {
      const safraDataToSave = {
        ...
        nome: safraData.nome,
        culturasPlantadas: safraData.culturasPlantadas
      };
        ...
        <FormContainer>
          <FormSection>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
              <FormGroup>
                <Label htmlFor="areaTotalHectares">Área Total (ha) *</Label>
                <Input
                  id="areaTotalHectares"
                  name="areaTotalHectares"
                  type="number"
                  value={fazendaData.areaTotalHectares || ''}
                  onChange={handleInputChange}
                  placeholder="0"
                  min="0"
                  step="0.01"
              ...
              <FormGroup>
                <Label htmlFor="areaAgricultavelHectares">Área Agricultável (ha)</Label>
                <Input
                  id="areaAgricultavelHectares"
                  name="areaAgricultavelHectares"
                  type="number"
                  value={fazendaData.areaAgricultavelHectares || ''}
                  onChange={handleInputChange}
                  placeholder="0"
                  min="0"
                  step="0.01"
              ...
              <FormGroup>
                <Label htmlFor="areaVegetacaoHectares">Área de Vegetação (ha)</Label>
                <Input
                  id="areaVegetacaoHectares"
                  name="areaVegetacaoHectares"
                  type="number"
                  value={fazendaData.areaVegetacaoHectares || ''}
                  onChange={handleInputChange}
                  placeholder="0"
                  min="0"
                  step="0.01"
